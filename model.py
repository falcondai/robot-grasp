...
def build_model(width):
    with tf.name_scope('model'):
        ...
        # rgb conv
        a1 = tf.contrib.layers.convolution2d(tf.nn.dropout(grasp_image_ph, keep_prob_ph), 32, (5, 5), tf.nn.relu, weight_init=tf.contrib.layers.xavier_initializer_conv2d(), name='conv1')
        ...
        # a3 = tf.contrib.layers.convolution2d(tf.nn.dropout(a2, keep_prob_ph), 128, (3, 3), tf.nn.relu, stride=(2, 2), weight_init=tf.contrib.layers.xavier_initializer_conv2d(), name='conv3')
        # a4 = tf.contrib.layers.convolution2d(tf.nn.dropout(a3, keep_prob_ph), 64, (3, 3), tf.nn.relu, stride=(2, 2), weight_init=tf.contrib.layers.xavier_initializer_conv2d(), name='conv4')
